...
<style>
    .main-header {
        background: linear-gradient(90deg, #528FF0, #7B68EE);
        padding: 1rem;
        border-radius: 10px;
        color: white;
        text-align: center;
...
<div class="main-header">
    <h1>📊 Support Ticket Analytics Dashboard</h1>
    <p>Comprehensive LoyaltyPro support operations intelligence with AI-powered insights</p>
</div>
""", unsafe_allow_html=True)
...
def process_csv(uploaded_file):
    ...
    try:
        ...
        # Convert date columns with multiple possible names
...
def main():
    ...
    if uploaded_file is None:
        ...
        <div class="upload-zone">
            <h3>Welcome to Support Ticket Analytics</h3>
            <p>Upload your CSV file to begin comprehensive ticket analysis</p>
            <p><strong>Features:</strong></p>
            <ul style="text-align: left; display: inline-block;">
                <li>30-day trend analysis</li>
                <li>AI-powered insights</li>
                <li>Individual ticket deep dives</li>
                <li>Client and priority breakdowns</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["30-Day Trends", "Search Tickets", "AI Insights", "Ticket Explorer"])
...
def show_ai_insights_tab(df):
    """Display AI-generated insights tab with structured tabular analysis"""
    st.header("AI-Powered Insights")
    
    # Display data scope information with date range
    date_info = ""
    if "Date" in df.columns:
        try:
            min_date = df["Date"].min()
            max_date = df["Date"].max()
            date_info = f" spanning from {min_date.strftime('%b %Y')} to {max_date.strftime('%b %Y')}"
        except:
            date_info = " (date range analysis unavailable)"
    
    st.info(f"📊 AI Analysis uses ALL historical data ({len(df)} records){date_info} - no 30-day restrictions applied. Product and client filters only.")
    ...
    if "Date" in df.columns and not df.empty:
        try:
            # Extract month-year for comparison, handling NaN values
            df['Month-Year'] = df["Date"].dt.strftime('%b-%Y')
            # Filter out NaN values before sorting
            month_values = df['Month-Year'].dropna().unique().tolist()
            months = sorted([x for x in month_values if pd.notna(x) and isinstance(x, str)], 
        ...
        if len(months) > 1:
            ...
            with col1:
                current_month = st.selectbox(
                    ...
                    options=months,
                    index=len(months)-1,
                    key="ai_insights_current_month"
                )
            
            with col2:
                previous_months = [m for m in months if m != current_month]
                previous_month = st.selectbox(
                    "Comparison period:", 
                    options=previous_months,
                    index=len(previous_months)-1 if previous_months else 0,
                    key="ai_insights_previous_month"
                )
            
            # Display selected date range
            st.info(f"Analyzing trends: {previous_month} vs. {current_month}")
        else:
            st.info(f"Only one month available: {months[0] if months else 'N/A'}")
    ...
    if st.button("Generate Tabulated Analysis", key="ai_insights_generate_button"):
        with st.spinner("Analyzing ticket data with Gemini AI..."):
            try:
                # Configure Gemini API
                genai.configure(api_key=api_key)
                ...
                # Prepare the data based on selected analysis type
                if analysis_type == "Jira Ticket Deep Dive":
                    # Use the specialized Jira analysis function
                    insights = generate_jira_insights_section(df, api_key)
                elif current_month and previous_month:
                    # Filter data for current and previous months
                    current_df = df[df['Month-Year'] == current_month].copy()
                    previous_df = df[df['Month-Year'] == previous_month].copy()
                    
                    if analysis_type == "Issue Category × Client":
                        prompt = generate_category_client_prompt(current_df, previous_df)
                    elif analysis_type == "Month-over-Month Comparison":
                        prompt = generate_mom_comparison_prompt(current_df, previous_df)
                    else:  # Root Cause & Action Plan
                        prompt = generate_action_plan_prompt(current_df, previous_df)
                    
                    # Call Gemini with the prompt
                    response = model.generate_content(prompt)
                    insights = response.text
                else:
                    # Use all data if specific months aren't available
                    prompt = generate_overall_analysis_prompt(df, analysis_type)
...
def show_search_tab(df):
    ...
    # Create two columns for the search field selector
    col1, col2 = st.columns([3, 1])
    with col2:
        search_field = st.selectbox(
            "Search in field:",
            options=["All Fields"] + df.columns.tolist(),
            key="search_tab_field_selector"
        )
...
def analyze_filtered_tickets(filtered_df, jira_col):
    ...
    with st.spinner("Analyzing filtered tickets with Gemini AI..."):
        try:
            ...
            prompt = f"""
            You are a support operations analyst for Razorpay's LoyaltyPro product. Analyze these {len(ticket_data)} tickets from the filtered search results and provide a concise pattern analysis.

            [FILTERED_TICKETS]
            {json.dumps(ticket_data, indent=2)}
            [/FILTERED_TICKETS]

            Provide a comprehensive summary with these EXACT sections:

            ## Filtered Tickets Summary

            **Total Tickets:** {len(ticket_data)}

            ### Common Patterns

            [3-5 bullet points identifying common patterns across these specific tickets]
            ...
            [3-5 bullet points with specific technical insights about these tickets]

            ### Recommended Actions

            [3-5 specific actionable recommendations to address these tickets]
            ...
            Format your analysis to be concise yet technically precise. Include specific details based on the ticket data. Focus on actionable insights.
            """
