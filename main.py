...
def filter_loyaltypro_data(df):
    ...
    if 'Product name' in df.columns:
        # Show available product names for debugging
        unique_products = df['Product name'].dropna().unique()
        st.sidebar.info(f"Available products: {', '.join(unique_products[:5])}")
        
        # Case-insensitive filtering
        loyaltypro_mask = df['Product name'].str.lower().str.contains('loyaltypro', na=False)
        df = df[loyaltypro_mask]
        st.info(f"Filtered to LoyaltyPro tickets: {len(df)} records")
    else:
        st.warning("No 'Product name' column found - showing all records")
    return df
...
def analyze_ticket_with_gemini(ticket_data):
    ...
    try:
        ...
        # LoyaltyPro-specific analysis prompt
        prompt = f"""
        You are analyzing support ticket data for LoyaltyPro, a product from Razorpay. This is a technical analysis task to identify patterns and provide insights from Jira tickets. Focus on extracting actionable intelligence from support ticket data.

        [TICKET DATA]
        {ticket_context}
        [/TICKET DATA]

        Your task is to analyze this ticket and provide the following:

        1. **Issue Summary (2-3 sentences)**
           - Concise description of the core problem

        2. **Technical Root Cause**
           - Identify the fundamental technical issue
           - Categorize as: API Error, Configuration Issue, Integration Problem, Data Inconsistency, Performance Issue, or User Error
           - Include confidence score (0-100%)

        3. **Resolution Path Analysis**
           - Evaluate the troubleshooting approach taken
           - Identify if the resolution was optimal or if there were more efficient approaches
           - Note any unnecessary steps or delays

        4. **Similar Issue Pattern**
           - Identify if this matches common patterns seen across LoyaltyPro tickets
           - Flag potential systemic issues that may affect other clients

        5. **Client-Specific Context**
           - Analyze if this issue is unique to this client or common across clients
           - Identify any client-specific configurations that contributed to the issue

        6. **Technical Preventive Measures**
           - Recommend 1-2 specific technical changes to prevent recurrence
           - Suggest monitoring or alerts that could catch this issue earlier

        7. **Business Impact Assessment**
           - Evaluate severity in terms of client experience
           - Estimate potential impact on revenue or client satisfaction
           - Score impact on a scale of 1-10

        Format your analysis as a structured report with clear headings and concise bullet points. Include a "TAGS" section at the end with 3-5 key tags that could be used to categorize this ticket.

        IMPORTANT: Stick to factual analysis based only on the provided ticket data. Do not invent details or make assumptions beyond what's in the ticket.
        """
...
if uploaded_file is not None:
    try:
        ...
        df = pd.read_csv(uploaded_file, encoding='utf-8', low_memory=False, dtype=str)
        
        # Debug information display
        st.subheader("Debug Information")
        st.write(f"**Total rows loaded from CSV:** {len(df)}")
        ...
        # DEBUG: Show product name values
        if "Product name" in df_filtered.columns:
            st.write("**Unique product names found:**", df_filtered["Product name"].unique())
            
            # Use flexible matching for LoyaltyPro
            loyalty_variations = ["LoyaltyPro", "Loyalty_Pro", "Loyalty Pro", "loyaltypro", "LOYALTYPRO"]
            loyalty_mask = df_filtered["Product name"].isin(loyalty_variations)
            loyalty_count = loyalty_mask.sum()
            st.write(f"**Found {loyalty_count} LoyaltyPro records**")
            
            df_filtered = df_filtered[loyalty_mask]
        else:
            st.error("Column 'Product name' not found in data")
        
        # Client filtering
        if "Account name" in df_filtered.columns:
            st.write("**Unique account names:**", df_filtered["Account name"].unique())
            
            # Target clients
            clients = ["AU Bank", "Axis", "DBS Bank", "Extraordinary Weekends", 
                     "Fi Money", "HDFC Bank", "IDFC FIRST Bank", "Jana Bank", 
                     "Kotak Mahindra Bank", "SBI Aurum"]
                     
            client_mask = df_filtered["Account name"].isin(clients)
            client_count = client_mask.sum()
            st.write(f"**Found {client_count} records matching target clients**")
            
            df_filtered = df_filtered[client_mask]
        else:
            st.error("Column 'Account name' not found in data")
        ...
        if jira_col:
            ...
            jira_mask = df_filtered[jira_col].notna() & (df_filtered[jira_col] != "")
            jira_count = jira_mask.sum()
            st.write(f"**Found {jira_count} records with Jira tickets**")
            
            # Extract ticket IDs for analysis
            jira_ticket_ids = df_filtered[jira_mask][jira_col].unique().tolist()
            
            df_filtered = df_filtered[jira_mask]
        else:
            st.error("No Jira ticket column found in data")
            jira_ticket_ids = []
        ...
        st.session_state['columns'] = df_filtered.columns.tolist()
        st.session_state['jira_tickets'] = jira_ticket_ids
        
        st.sidebar.success(f"✅ Loaded {len(df_filtered)} filtered records")
        if jira_ticket_ids:
